 ...
 * Enforces addon dependencies with semver comparison.
 */
...
/**
 * Check if a version satisfies a semver constraint (e.g. ">=1.0.0")
 */
function satisfiesSemver(version, constraint) {
...
class AddonManager {
    ...
    for (const [id, addon] of this.addons) {
      ...
      if (addon.manifest.navItems) {
        for (const item of addon.manifest.navItems) {
          this._navItems.push({ ...item, addonId: addon.id, addonType: addon.type });
        }
      }
  ...
  /**
   * Get hard dependencies for an addon as {addonId: semverConstraint}
   * Normalizes both object and array formats.
   */
  getDependencies(addonId) {
    const addon = this.addons.get(addonId);
    if (!addon) return {};
    const deps = addon.manifest.dependencies;
    if (!deps) return {};
    if (Array.isArray(deps)) {
      // Legacy format: ["maps", "campaigns"] -> {"maps": ">=1.0.0", ...}
      const result = {};
      for (const id of deps) {
        if (typeof id === 'string' && id) result[id] = '>=1.0.0';
      }
      return result;
    }
    if (typeof deps === 'object') return deps;
    return {};
  }

  /**
   * Get soft dependencies for an addon as {addonId: semverConstraint}
   */
  getSoftDependencies(addonId) {
    const addon = this.addons.get(addonId);
    if (!addon) return {};
    const deps = addon.manifest.softDependencies;
    if (!deps) return {};
    if (Array.isArray(deps)) {
      const result = {};
      for (const id of deps) {
  ...
  /**
   * Check if all hard dependencies for an addon are satisfied.
   * Returns { satisfied: true } or { satisfied: false, missing: [...] }
   */
  checkDependencies(addonId) {
    const deps = this.getDependencies(addonId);
    const missing = [];

    for (const [depId, constraint] of Object.entries(deps)) {
      const depAddon = this.addons.get(depId);
      if (!depAddon) {
        missing.push({ id: depId, constraint, reason: 'not_found', name: depId });
      } else if (!depAddon.enabled) {
        missing.push({
          id: depId, constraint, reason: 'disabled',
          name: depAddon.name, version: depAddon.version,
          type: depAddon.type
        });
      } else if (!satisfiesSemver(depAddon.version, constraint)) {
        missing.push({
          id: depId, constraint, reason: 'version_mismatch',
          name: depAddon.name, version: depAddon.version,
          type: depAddon.type
        });
      }
  ...
  /**
   * Get all addons that depend on the given addon (reverse dependencies).
   * Only returns addons that are currently enabled.
   */
  getDependents(addonId) {
    const dependents = [];
    for (const [id, addon] of this.addons) {
      if (!addon.enabled) continue;
      const deps = this.getDependencies(id);
      if (deps[addonId]) {
        dependents.push({ id, name: addon.name, version: addon.version });
      }
    }
    return dependents;
  }
  ...
  /**
   * Detect circular dependencies starting from addonId.
   * Returns array of addon IDs forming the cycle, or empty array if no cycle.
   */
  detectCircularDependencies(addonId) {
  ...
  /**
   * Resolve the full dependency chain for an addon (including transitive deps).
   * Returns ordered list of {id, action} where action is 'enable' or 'install'.
   */
  resolveDependencyChain(addonId) {
    const chain = [];
  ...
  /**
   * Enable an addon (does NOT check dependencies — caller must check first)
   */
  enable(addonId) {
    const addon = this.addons.get(addonId);
  ...
  /**
   * Enable an addon with dependency enforcement.
   * Checks that all hard dependencies are enabled first.
   * Returns { success: true } or { success: false, missing: [...] }
   */
  enableWithDependencyCheck(addonId) {
    const check = this.checkDependencies(addonId);
    if (!check.satisfied) {
      return { success: false, missing: check.missing };
    }

    // Check for circular deps
    const cycle = this.detectCircularDependencies(addonId);
    if (cycle.length > 0) {
      return { success: false, circular: cycle };
    }
  ...
  /**
   * Disable an addon with dependency enforcement.
   * Blocks if other enabled addons depend on it.
   * Returns { success: true } or { success: false, dependents: [...] }
   */
  disableWithDependencyCheck(addonId) {
    const dependents = this.getDependents(addonId);
  ...
  /**
   * Disable an addon (no dependency check)
   */
  disable(addonId) {
    const addon = this.addons.get(addonId);
    if (!addon) throw new Error(`Addon not found: ${addonId}`);
  ...
  /**
   * Clean up addon metadata (migration records only).
   * SAFETY: We intentionally do NOT run down migrations on uninstall.
   * Addon tables are left in the database to preserve user data.
   * Down migrations are only used for development/testing.
   */
  deleteData(addonId) {
    const addon = this.addons.get(addonId);
    if (!addon) throw new Error(`Addon not found: ${addonId}`);

    // SAFE: Only remove migration tracking records, NOT the actual tables/data.
    // Addon tables remain in the database — they are harmless empty tables
    // and preserve any user-created data that references them.
    console.log(`[AddonManager] Cleaning up metadata for ${addonId} (tables preserved)`);
    this.db.prepare('DELETE FROM addon_migrations WHERE addon_id = ?').run(addonId);
  }

  /**
   * Uninstall a community addon with dependency check.
   * Blocks if other enabled addons depend on it.
   * Returns { success: true } or { success: false, dependents: [...] }
   */
  uninstallWithDependencyCheck(addonId) {
    const dependents = this.getDependents(addonId);
    if (dependents.length > 0) {
      return { success: false, dependents };
    }
    this.uninstall(addonId);
    return { success: true };
  }
...
module.exports.satisfiesSemver = satisfiesSemver;
